...
export default {
  ...
  data() {
    return {
      ...
      editId: 0,
      deleteId: 0,
      companyValidationSchema: yup.object({
  ...
  methods: {
    ...
    editModalToggle(companyId) {
      this.editId = companyId;
      this.showEditModal = true;
    },

    async editCompanyHandler(values) {
      try {
        const res = await this.editCompany(this.editId, values);
    ...
    <v-table class="rounded flex bg-gray-100 shadow">
      ...
      <tbody>
        <tr v-for="company in companies" :key="company.id">
          <td class="text-center text-nowrap">{{ company.name }}</td>
    ...
    >
      ...
      <v-card class="bg-green-100 flex flex-col rounded p-4 w-3/6 h-2/6">
        <v-card-text class="flex flex-col justify-center">
          <Form
            :validation-schema="companyValidationSchema"
